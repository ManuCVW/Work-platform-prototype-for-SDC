<aside class="sidebar">
    <div class="contenedor-sidebar">
        <h2>WorkWise</h2>

        <div class="cerrar-menu">
            <img id="cerrar-menu" src="build/img/cerrar.svg" alt="imagen cerrar menu">
        </div>
    </div>
    

    <nav class="sidebar-nav">
        <a class="<?php echo ($titulo === 'Proyectos') ? 'activo' : ''; ?>" href="/dashboard">Proyectos</a>
        <?php if($_SESSION["tipoUsuario"] === "1" || $_SESSION["tipoUsuario"] === "2"): ?>
            <a class="<?php echo ($titulo === 'Crear Proyecto') ? 'activo' : ''; ?>" href="/crear-proyecto">Crear Proyecto</a>
        <?php endif; ?>

        <?php if($_SESSION["tipoUsuario"] === "1" || $_SESSION["tipoUsuario"] === "2"): ?>
            <a class="<?php echo ($titulo === 'Crear Usuario') ? 'activo' : ''; ?>" href="/crear-usuario">Crear Usuario</a>
        <?php endif; ?>

        <?php if($_SESSION["tipoUsuario"] < 3): ?>
            <a class="<?php echo ($titulo === 'Administracion de Chats') ? 'activo' : ''; ?>" href="/administracion-chats">Administracion de Chats</a>
        <?php endif; ?>


        <a class="<?php echo ($titulo === 'Perfil') ? 'activo' : ''; ?>" href="/perfil">Perfil</a>
    </nav>

    <div class="cerrar-sesion-mobile">
        <a href="/logout" class="cerrar-sesion">Cerrar Sesión</a>
    </div>
</aside>